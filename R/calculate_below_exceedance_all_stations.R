#' Calculate Stations with Annual Average Below Threshold and Their Exceedance Days
#'
#' This function calculates the annual average concentration of a specified air quality parameter
#' for stations that have a data availability percentage below 90%. It returns
...
#' @return A data frame containing the stations with an annual average below the specified threshold and the number of exceedance days.
#' @export
#'
#' @examples
#' # Calculate stations with annual average PM10 below 40 µg/m³ and at least 90% data availability
#' result <- calculate_below_exceedance_days_all_stations("PM10", 40)
#' print(result)

calculate_below_exceedance_all_stations <- function(parameter, data_type, pollutant_threshold, data_threshold = 90, until_year = 2023) {
  ...
  process_data <- function(data, total_amount) {
    ...
    # Count total and available data per station per year below threshold
    data_summary <- data %>%
