...
calculate_overall_average_by_city_threshold <- function(parameter) {
  ...
  city_query <- dbGetQuery(mydb, paste0("SELECT DISTINCT Sehir FROM location_2023", YEAR))
  cities <- city_query$Sehir

  overall_avgs <- sapply(cities, function(city_name) {
    station_query <- dbGetQuery(mydb, paste0("SELECT Istasyonlar FROM location_2023", YEAR, " WHERE Sehir='", city_name, "'"))
