...
daily_count_stations_with_parameter_threshold <- function(parameter_name, threshold = 90, season = NULL) {
  ...
  query <- paste0("SELECT COUNT(DISTINCT Istasyon) AS station_count
                   FROM daily_detail
                   WHERE 1=1 ", season_filter, "
                   GROUP BY Istasyon
                   HAVING (SUM(CASE WHEN ", parameter_name, " IS NOT NULL THEN 1 ELSE 0 END) * 100 / ", days_in_season, ") >= ", threshold)
