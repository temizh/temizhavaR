...
#'
#' @return A `data.frame` containing the station name (`Istasyon_modified`), timestamp (`Tarih`), and
...
calculate_rolling_8hour_avg_CO <- function() {
  ...
  hourly_data <- dbGetQuery(conn,
                            "SELECT Istasyon_modified, Tarih, CO
                             FROM hourly_detail
                             WHERE Tarih BETWEEN '2023-01-01 00:00:56' AND '2024-01-01 00:00:56'
                             ORDER BY Istasyon_modified, Tarih")
  ...
  filtered_data <- hourly_data %>%
    filter(Istasyon_modified %in% station_data_availability$Istasyon_modified)
  ...
  result <- filtered_data %>%
    ...
    ungroup() %>%
    select(Istasyon_modified, Tarih, CO_8hour_avg)
