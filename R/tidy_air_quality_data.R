...
tidy_air_quality_data <- function(tbl_db, table_name, check_hourly = FALSE, con = NULL, update_original = TRUE) {
  ...
  tryCatch({
    ...
    if (!should_clean) {
      log_to_db("No data quality issues detected before cleaning modifications; proceeding for consistency.", "INFO", "DATA_CHECK", tbl_db %>% pull(location_id) %>% unique() %>% as.character(), NULL, NULL)
    }
    ...
    tryCatch({
      dbExecute(con, "CREATE TABLE IF NOT EXISTS data_cleaning_log ( id SERIAL PRIMARY KEY, session_id TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, script_name TEXT, log_level TEXT, category TEXT, location_id TEXT, station_name TEXT, message TEXT, details JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP )")
      dbExecute(con, "CREATE TABLE IF NOT EXISTS data_quality_log ( id SERIAL PRIMARY KEY, table_name TEXT, location_id TEXT, operation_type TEXT, field_name TEXT, affected_rows INTEGER, reason TEXT, old_value TEXT, new_value TEXT, operation_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP )")
    }, error = function(e) { warning("Failed to create log tables: ", e$message); return(NULL) })
    ...
    log_to_db <- function(message, level, category, location_id = NULL, station_name = NULL, details = NULL) {
      ...
      details_json <- tryCatch({ if (is.null(details)) { "null" } else if (is.list(details)) { jsonlite::toJSON(details, auto_unbox = TRUE) } else { jsonlite::toJSON(list(value = details), auto_unbox = TRUE) } }, error = function(e) { warning("Failed to convert details to JSON: ", e$message); "null" })
      tryCatch({
        ...
        dbExecute(con, sql)
      }, error = function(e) { warning("Failed to write to database log: ", e$message) })
      timestamp <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
      cat(sprintf("[%s] %s | %s | %s\n", timestamp, level, ifelse(is.null(station_name), category, station_name), message))
    }
    if (negative_values > 0) {
      log_to_db(sprintf("Found %d records with negative values", negative_values), "WARN", "QUALITY_CHECK", tbl_db %>% pull(location_id) %>% unique() %>% as.character(), tbl_db %>% pull(Istasyon_modified) %>% unique() %>% as.character(), list(check_type = "negative_values", affected_count = negative_values, table_name = table_name))
    }
    log_quality_operation <- function(operation_type, field_name, affected_rows, reason, old_value = NULL, new_value = NULL) {
      dbExecute(con, sprintf(" INSERT INTO data_quality_log (table_name, location_id, operation_type, field_name, affected_rows, reason, old_value, new_value) VALUES (%s, %s, %s, %s, %d, %s, %s, %s)",
      ...
      log_to_db(sprintf("Quality check: %s %s records affected", operation_type, affected_rows), "INFO", "QUALITY_CHECK", tbl_db %>% pull(location_id) %>% unique() %>% as.character(), NULL, list(field = field_name, reason = reason, affected_rows = affected_rows))
    }
    ...
    if (update_original) {
      ...
      if (issues_found > 0) {
          ...
          if (nrow(all_old_backups) > 0) {
            for (old_backup in all_old_backups$table_name) {
              tryCatch({
                ...
                log_to_db(sprintf("Removed old backup table %s for location %s", old_backup, location_id), "INFO", "DATA_BACKUP", location_id, NULL, list(location_id = location_id, removed_backup = old_backup))
              }, error = function(e) {
                log_to_db(sprintf("Failed to remove old backup table %s: %s", old_backup, e$message), "WARN", "DATA_BACKUP", location_id, NULL, list(location_id = location_id, backup_error = e$message))
              })
            }
          }
          log_to_db(sprintf("Created backup of original data for location %s as %s", location_id, backup_table), "INFO", "DATA_BACKUP", location_id, NULL, list(location_id = location_id, backup_table = backup_table))
        } else {
          log_to_db(sprintf("Using existing backup %s for location %s from today", backup_table, location_id), "INFO", "DATA_BACKUP", location_id, NULL, list(location_id = location_id, backup_table = backup_table))
        }
      } else {
        dbExecute(con, sprintf("ALTER TABLE %s DROP COLUMN IF EXISTS needs_cleaning", dbQuoteIdentifier(con, temp_table)))
        previous_cleanings <- dbGetQuery(con, sprintf(" SELECT MAX(operation_time) as last_cleaned_at, COUNT(*) as clean_operations, SUM(CASE WHEN field_name = 'PM10,PM25' THEN affected_rows ELSE 0 END) as pm_fixes, SUM(CASE WHEN field_name = 'NO,NO2,NOX' THEN affected_rows ELSE 0 END) as nox_fixes, SUM(CASE WHEN reason = 'negative_values' THEN affected_rows ELSE 0 END) as negative_fixes FROM data_quality_log WHERE location_id = %s AND table_name = %s AND operation_type = 'nullify' GROUP BY location_id, table_name", dbQuoteString(con, as.character(location_id)), dbQuoteString(con, table_name)))
        if (nrow(previous_cleanings) > 0 && previous_cleanings$clean_operations[1] > 0) {
          ...
          log_to_db(sprintf("No new issues found for location %s, but previously cleaned data exists. Last cleaned: %s", location_id, last_cleaned), "INFO", "DATA_HISTORY", location_id, NULL, list(location_id = location_id, last_cleaned = last_cleaned, pm_fixes = previous_cleanings$pm_fixes[1], nox_fixes = previous_cleanings$nox_fixes[1], negative_fixes = previous_cleanings$negative_fixes[1], total_operations = previous_cleanings$clean_operations[1]))
          if (previous_cleanings$pm_fixes[1] > 0) {
            log_to_db(sprintf("Previously fixed %.0f records with PM2.5 > PM10 relationship issues", previous_cleanings$pm_fixes[1]), "INFO", "HISTORICAL_FIXES", location_id, NULL, NULL)
          }
          if (previous_cleanings$nox_fixes[1] > 0) {
            log_to_db(sprintf("Previously fixed %.0f records with invalid NOx relationships", previous_cleanings$nox_fixes[1]), "INFO", "HISTORICAL_FIXES", location_id, NULL, NULL)
          }
          if (previous_cleanings$negative_fixes[1] > 0) {
            log_to_db(sprintf("Previously fixed %.0f records with negative values", previous_cleanings$negative_fixes[1]), "INFO", "HISTORICAL_FIXES", location_id, NULL, NULL)
          }
        } else {
