...
#' @param update_original Whether to update the original table with cleaned data
...
tidy_air_quality_data <- function(tbl_db, table_name, check_hourly = FALSE, con = NULL, update_original = TRUE) {
  ...
  tryCatch({
    ...
    tryCatch({
      ...
      # Add needs_cleaning column after table creation
      dbExecute(con, sprintf('ALTER TABLE %s ADD COLUMN needs_cleaning BOOLEAN',
                           dbQuoteIdentifier(con, temp_table)))
    }, error = function(e) {
    ...
    # Move needs_cleaning function and check to the beginning, right after temp table creation
    needs_cleaning <- function(temp_table) {
      # Check all quality issues before any cleaning is done
      checks <- dbGetQuery(con, sprintf("
        SELECT 
          (SELECT COUNT(*) FROM %1$s 
           WHERE \"PM25\" > \"PM10\" AND \"PM25\" IS NOT NULL AND \"PM10\" IS NOT NULL) > 0 
          OR (SELECT COUNT(*) FROM %1$s 
              WHERE (\"NO\" + \"NO2\") NOT BETWEEN \"NOX\" * 0.5 AND \"NOX\" * 1.5 
              AND \"NO\" IS NOT NULL AND \"NO2\" IS NOT NULL AND \"NOX\" IS NOT NULL) > 0
          OR (SELECT COUNT(*) FROM %1$s 
              WHERE (\"PM10\" < 0 AND \"PM10\" IS NOT NULL)
                 ...
                 OR (\"NOX\" < 0 AND \"NOX\" IS NOT NULL)) > 0 as needs_cleaning", 
        temp_table))
      
      return(as.logical(checks$needs_cleaning))
    }

    # Check if cleaning is needed before any operations
    should_clean <- needs_cleaning(temp_table)
    
    if (!should_clean) {
      log_to_db(
        "No data quality issues detected before cleaning modifications; proceeding for consistency.",
        "INFO",
        "DATA_CHECK",
    ...
    # Only continue with these operations if cleaning is needed
    ...
    # Compute cleaned_count before any column drop operations
    ...
    if (update_original) {
      ...
      # Check if any existing backup exists for this location and table
      existing_backups <- dbGetQuery(con, sprintf("
        SELECT table_name, created_at 
        FROM (
          SELECT table_name,
                 (regexp_matches(table_name, '%s_backup_(\\d{8}_\\d{6})'))::text[] AS backup_date,
                 to_timestamp(regexp_replace(table_name, '.*_backup_(\\d{8}_\\d{6}).*', '\\1'), 'YYYYMMDD_HH24MISS') as created_at
          FROM information_schema.tables 
          WHERE table_name LIKE '%s_backup_%%'
      ...
      if (nrow(existing_backups) > 0) {
        # Check if the most recent backup was made today
        ...
        # Only consider backups from today as "recent"
        has_recent_backup <- (today == backup_day)
        if (has_recent_backup) {
          backup_table <- most_recent_backup
        }
      }
      
      if (issues_found > 0) {
        # Create a backup if we don't have a recent one or if it's a new day
        if (!has_recent_backup) {
          backup_table <- paste0(table_name, "_backup_", format(Sys.time(), "%Y%m%d_%H%M%S"))
          
          # First, collect all existing backups for this location
          all_old_backups <- dbGetQuery(con, sprintf("
            SELECT table_name
            FROM information_schema.tables
            WHERE table_name LIKE '%s_backup_%%'
            AND table_schema = 'public'
          ...
          dbExecute(con, sprintf("
            CREATE TABLE %s AS 
            SELECT *, %s::boolean as needs_cleaning 
            FROM %s 
            WHERE CAST(location_id AS TEXT) = %s", 
            dbQuoteIdentifier(con, backup_table),
            dbQuoteString(con, "TRUE"),
            dbQuoteIdentifier(con, table_name),
            dbQuoteString(con, as.character(location_id))))
          
          # Drop old backup tables after creating the new one
          if (nrow(all_old_backups) > 0) {
            for (old_backup in all_old_backups$table_name) {
              tryCatch({
                ...
                log_to_db(
                  sprintf("Removed old backup table %s for location %s", old_backup, location_id),
                  "INFO",
                  "DATA_BACKUP",
                  location_id,
                  NULL,
                ...
                log_to_db(
                  sprintf("Failed to remove old backup table %s: %s", old_backup, e$message),
                  "WARN",
                  "DATA_BACKUP",
                  location_id,
                  NULL,
          ...
          log_to_db(
            sprintf("Created backup of original data for location %s as %s", location_id, backup_table),
            "INFO",
            "DATA_BACKUP",
            location_id,
            NULL,
          ...
          log_to_db(
            sprintf("Using existing backup %s for location %s from today", backup_table, location_id),
            "INFO",
            "DATA_BACKUP",
            location_id,
            NULL,
        ...
        # When no issues are found, check if data was previously cleaned
        dbExecute(con, sprintf("ALTER TABLE %s DROP COLUMN IF EXISTS needs_cleaning", 
                           dbQuoteIdentifier(con, temp_table)))
        
        # Query logs to check if this location had data cleaning in the past
        previous_cleanings <- dbGetQuery(con, sprintf("
          SELECT 
            MAX(operation_time) as last_cleaned_at,
            COUNT(*) as clean_operations,
            SUM(CASE WHEN field_name = 'PM10,PM25' THEN affected_rows ELSE 0 END) as pm_fixes,
            SUM(CASE WHEN field_name = 'NO,NO2,NOX' THEN affected_rows ELSE 0 END) as nox_fixes,
            SUM(CASE WHEN reason = 'negative_values' THEN affected_rows ELSE 0 END) as negative_fixes
        ...
        if (nrow(previous_cleanings) > 0 && previous_cleanings$clean_operations[1] > 0) {
          # Format the last cleaned date for display
          last_cleaned <- format(previous_cleanings$last_cleaned_at[1], "%Y-%m-%d %H:%M:%S")
          
          log_to_db(
            sprintf("No new issues found for location %s, but previously cleaned data exists. Last cleaned: %s", 
                   location_id, last_cleaned),
            "INFO",
            "DATA_HISTORY",
            location_id,
            NULL,
            list(
              ...
              pm_fixes = previous_cleanings$pm_fixes[1],
              nox_fixes = previous_cleanings$nox_fixes[1],
              negative_fixes = previous_cleanings$negative_fixes[1],
          ...
          if (previous_cleanings$pm_fixes[1] > 0) {
            log_to_db(
              sprintf("Previously fixed %.0f records with PM2.5 > PM10 relationship issues", 
                     previous_cleanings$pm_fixes[1]),
              "INFO",
              "HISTORICAL_FIXES",
              location_id,
              NULL,
          ...
          if (previous_cleanings$nox_fixes[1] > 0) {
            log_to_db(
              sprintf("Previously fixed %.0f records with invalid NOx relationships", 
                     previous_cleanings$nox_fixes[1]),
              "INFO",
              "HISTORICAL_FIXES",
              location_id,
              NULL,
          ...
          if (previous_cleanings$negative_fixes[1] > 0) {
            log_to_db(
              sprintf("Previously fixed %.0f records with negative values", 
                     previous_cleanings$negative_fixes[1]),
              "INFO",
              "HISTORICAL_FIXES",
              location_id,
              NULL,
          ...
          log_to_db(
            sprintf("No data quality issues found for location %s. No previous cleaning records found.", location_id),
            "INFO",
            "DATA_CHECK",
            location_id,
            NULL,
    ...
    # Create the table reference after dropping the column
