...
count_stations_with_parameter_threshold <- function(parameter_name, data_type = "daily", threshold = 90, 
  ...
  tryCatch({
    ...
    if(nrow(data) == 0) {
      return(create_empty_result(threshold, data_type))
    }
    
    data$date <- as.Date(as.numeric(data$Tarih), origin = "1900-01-01")
    ...
    full_years <- data.frame(year = 2014:2023)
...
create_empty_result <- function(threshold, data_type = "daily") {
  period_col <- if(data_type == "daily") "days_in_period" else "hours_in_period"
  result <- data.frame(
    year = 2014:2023,
    station_count = 0,
