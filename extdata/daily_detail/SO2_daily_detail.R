...
output$SO2_3$data <- count_stations_with_parameter_threshold(parameter_name, "daily", threshold = 90)

output$SO2_4$result_message <- print(paste0(parameter_name, "_4 : için istasyon ortalamaları"))
output$SO2_4$data <- station_average(parameter_name, "daily", threshold = 90)
...
output$SO2_7$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 40)
...
output$SO2_8$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 125)
...
output$SO2_9$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 125, exceedance_count = 3)
...
output$SO2_11$data <- list_stations_with_parameter(parameter_name, "daily", data_threshold = 20)
