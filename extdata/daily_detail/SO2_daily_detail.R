...
result_so2_daily_excel_file <- file.path(output_dir, "results_SO2_daily.xlsx")
...
output$SO2_1$data <- list_stations_with_parameter(parameter_name, data_type = "daily", until_year = 2024)

output$SO2_2$result_message <- print(paste0(parameter_name, "_2 : %90 veri alınan istasyon listesi" ))
output$SO2_2$data <- list_stations_with_parameter(parameter_name, data_type = "daily", threshold = 90, until_year = 2024)

output$SO2_3$result_message <- print(paste0(parameter_name, "_3 : %90 Veri alınan istasyon sayısı" ))
output$SO2_3$data <- count_stations_with_parameter_threshold(parameter_name, "daily", threshold = 90, until_year = 2024)

output$SO2_4$result_message <- print(paste0(parameter_name, "_4 : için istasyon ortalamaları"))
output$SO2_4$data <- station_average(parameter_name, "daily", threshold = 90, until_year = 2024)
...
output$SO2_7$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 40, until_year = 2024)
...
output$SO2_8$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 125, until_year = 2024)
...
output$SO2_9$data <- calculate_above_exceedance_all_stations(parameter_name, "daily", pollutant_threshold = 125, exceedance_count = 3, until_year = 2024)

# Same as _4 ???
# output$SO2_10$result_message <- print(paste0(parameter_name, "_10 : için istasyon ortalamaları"))
# output$SO2_10$data <- daily_station_average(parameter_name, threshold = 90, until_year = 2024)

output$SO2_11$result_message <- print(paste0(parameter_name, "_11 : Yıllık ortalaması 20 µg/m3 üstü istasyonların listesi (veri yüzdesi)" ))
output$SO2_11$data <- list_stations_with_parameter(parameter_name, "daily", data_threshold = 20, until_year = 2024)
