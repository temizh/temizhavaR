...
output$NO2_5$result_message  <- print(paste(parameter_name,": Günlük ortalaması 25 esik degerini 3 kere asan istasyonlar ve kaç gün boyunca" ))
output$NO2_5$data <- daily_above_exceedance_days_double_threshold(parameter_name, threshold = 25, 3)

output$NO2_6$result_message <- print(paste(parameter_name, ": için istasyon ortalamaları"))
output$NO2_6$data <- daily_station_average(parameter_name, threshold = 90)

output$NO2_7$result_message <- print(paste(parameter_name," : Yıllık ortalaması 40 µg/m3 üstü istasyonların listesi ve kaç gün boyunca " ))
output$NO2_7$data <- calculate_above_exceedance_days_all_stations(parameter_name, threshold = 40)

output$NO2_8$result_message <- print(paste(parameter_name," : Yıllık ortalaması 10 µg/m3 üstü istasyonların listesi ve kaç gün boyunca " ))
output$NO2_8$data <- calculate_above_exceedance_days_all_stations(parameter_name, threshold = 10)

output$NO2_9$result_message <- print(paste(parameter_name,": İl NO2 yıllık ortalaması" ))
