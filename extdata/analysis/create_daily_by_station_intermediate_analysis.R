...
# Collect results
analysed_data <- analysed_data %>% collect()

# Write to database
#
# FIX 
#
dbWriteTable(conn, "daily_by_station_intermediate_analysis", analysed_data, overwrite = TRUE)

test <- tbl(conn, "daily_by_station_intermediate_analysis")
